...
export default function Page() {
  ...
  return (
    <>
      ...
      <div className="mt-24 py-8 flex flex-col items-center z-[600] relative lg:w-[980px] mx-auto space-y-6">
        <div className="absolute left-[50%] top-0 -translate-x-[50%] h-full w-1 bg-yellow-300 z-[600]"></div>
...
const RoadmapItem = ({
  ...
  <div
    className={`text-white max-w-md rounded-xl border-2 border-slate-900 bg-slate-800 ${
      alignLeft ? "lg:mr-auto" : "lg:ml-auto"
    }`}
  >
    <div
      className={`flex flex-row items-center justify-between ${
        alignLeft ? "lg:flex-row-reverse" : "lg:flex-row"
      }`}
    >
      <div
        className={`bg-yellow-300  px-3 py-1 w-max text-zinc-800 shadow-sm shadow-black ${
          alignLeft
            ? "rounded-br-xl rounded-tl-xl lg:rounded-bl-xl lg:rounded-tr-xl"
            : "rounded-br-xl rounded-tl-xl lg:rounded-br-xl lg:rounded-tl-xl"
        }`}
      >
        {item.year}
      </div>
