...
export default function Page() {
  ...
  return (
    <>
      ...
      <StarsBg />
      <div className="mt-24 w-full py-8 flex flex-row flex-wrap z-[600] relative space-x-6">
        {timeline.map((item: any, i: any) => (
          <div
            key={i}
            className="text-white rounded-xl border-2 border-slate-900 bg-slate-800"
          >
            <div className="flex flex-row items-center justify-between">
              <div className="bg-yellow-300 px-3 py-1 rounded-br-xl rounded-tl-xl w-max text-zinc-800 shadow-sm shadow-black">
                {item.year}
              </div>
              <h2 className="pr-3">{item.title}</h2>
            </div>
