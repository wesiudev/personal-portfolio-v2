...
export default function Page() {
  ...
  return (
    <>
      ...
      >
        <div className="bg-slate-800 p-3 py-12 border-2 border-slate-900 md:w-max md:mx-auto md:px-12 md:rounded-3xl md:rounded-tl-none relative">
          <div className="absolute md:left-[50%] top-0 md:-translate-x-[50%] h-full w-1 bg-yellow-300 z-[0] -top-12 rounded-full"></div>

          <div className="md:w-[675px] lg:w-[900px] mx-auto">
            <h2 className="text-[#F7E11F] drop-shadow-xl shadow-black text-3xl lg:text-5xl xl:text-6xl 2xl:text-7xl font-bold">
              Motivation
            </h2>
            <p className="text-white font-dosis text-base lg:text-xl mt-3">
              I find immense satisfaction in working with data that I can
              visualize. Observing the flow of information between the
              components I create is a source of great fascination for me. I am
              captivated by the logic of applications and the optimization
              process. Designing and implementing new features excites me, and I
              am constantly driven to innovate. My passion for programming fuels
              my desire for continuous growth, and I actively pursue
              opportunities to further develop my skills.
            </p>
          </div>
        </div>
        <div className="text-white text-lg md:text-2xl space-y-12 w-[80vw] md:w-[70vw] xl:w-[60vw] mt-16">
          <h3></h3>
          <h3></h3>
        </div>
...
const RoadmapItem = ({
  ...
  <div
    className={`text-white w-[250px] sm:w-[400px] md:w-[300px] lg:w-[410px] rounded-2xl border-2 border-slate-900 bg-slate-800 ${
      alignLeft
  ...
  >
    ...
    >
      <div
        className={`font-anta bg-yellow-300 rounded-br-xl rounded-tl-xl px-3 py-1 w-max text-zinc-800 ${
          alignLeft
            ? "md:rounded-bl-xl md:rounded-br-none md:rounded-tl-none"
            : "rounded-br-xl md:rounded-l-none"
        }`}
      >
        {item.year}
      </div>
