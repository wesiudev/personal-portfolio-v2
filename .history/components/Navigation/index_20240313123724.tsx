...
import AOS from "aos";
export function NavRight({ dictionary }: { dictionary: any }) {
  ...
  return (
    <div className="text-gray-300 fixed right-6 lg:right-12 top-12 flex flex-row space-x-3 z-[1000] font-regular text-xl font-anta">
      <ScrollTo
        to="projects"
        smooth={true}
        duration={500}
        ...
        className={`group h-max overflow-hidden cursor-pointer flex flex-row items-center ${
          !pathname.includes("/about") && pathname !== "/blog"
            ? ""
            : "absolute -translate-x-[200vw]"
        }`}
      >
