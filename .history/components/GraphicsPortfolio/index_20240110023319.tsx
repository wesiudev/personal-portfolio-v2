...
const GraphicsPortfolio: React.FC = () => {
  ...
  const donutPosY = useTransform(scrollYProgress, [0.1, 0.7, 1], [1, 0, -5]);
  ...
  return (
    <>
      <motionDiv.div
        className={`${
          !useTransform(scrollYProgress, (pos) => (pos >= 0.9 ? true : false))
            ? "opacity-100 translate-y-0"
            : "opacity-0 translate-y-24"
        } duration-300 fixed left-[50%] top-[50%] -translate-x-[50%] -translate-y-[50%] z-[503]`}
      >
        <Link
          data-aos="fade-up"
          href="#"
          className="font-sans font-bold italic text-4xl bg-yellow-300"
        >
          Projects Showcase
        </Link>
      </motionDiv.div>
