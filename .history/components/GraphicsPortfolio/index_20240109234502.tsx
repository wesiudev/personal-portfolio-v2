...
const HeroDonut = ({ gltfPath
  donut2PosX,
  donut2PosY,
  donut2PosZ,
}: { gltfPath: string donut2PosX:number
  donut2PosY:number
  donut2PosZ:number }) => {
  const gltf = useLoader(GLTFLoader, gltfPath);
...
const GraphicsPortfolio: React.FC = () => {
  ...
  const scale = useTransform(scrollYProgress, [0.2, 0.6, 1], [0, 2, 20]);
  ...
  const donut2PosX = useTransform(scrollYProgress, [0.1, 0.3], [0, 1]);
  const donut2PosY = useTransform(scrollYProgress, [0.1, 0.5], [1, 0]);
  const donut2PosZ = useTransform(scrollYProgress, [0.1, 0.5], [1, 3]);
