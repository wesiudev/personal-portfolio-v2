...
const HeroDonut = ({ gltfPath }: { gltfPath: string }) => {
  ...
  useFrame(() => (mesh.current.rotation.y += 1));
  return (
    <motion.primitive
      ref={mesh}
      object={gltf.scene}
      ...
      position={[3, -2, 0]}
      animate={{
        rotateY: 360, // Rotate 360 degrees around Y-axis
      }}
      transition={{
        duration: 5, // 5 seconds for each full rotation
...
const GraphicsPortfolio: React.FC = () => {
  ...
  const gltfPath2 = "/untitled4.glb"; // Replace with the actual path to your GLB model
