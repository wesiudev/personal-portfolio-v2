...
const GraphicsPortfolio: React.FC = () => {
  ...
  const scale = useTransform(scrollYProgress, [0.2, 0.6, 1], [0, 2, 40]);
  const rotateX = useTransform(scrollYProgress, [0.1, 0.7, 0.8], [0, 8.5, 9.3]);
  const rotateY = useTransform(scrollYProgress, [0.4, 0.7], [0, 3]);
  ...
  const donutPosZ = useTransform(scrollYProgress, [0.1, 0.5], [1, 12]);

  const donut2PosX = useTransform(scrollYProgress, [0.1, 0.3], [3, 6]);
  const donut2PosY = useTransform(scrollYProgress, [0.1, 0.5], [-1, -5]);
  const donut2PosZ = useTransform(scrollYProgress, [0.1, 0.5], [0, 7]);

  const background = useTransform(scrollYProgress, (pos) =>
    pos >= 0.9 ? "rgb(63 63 70)" : ""
  );
  ...
  return (
    <>
      <motionDiv.div
        className="fixed left-0 top-0 z-[501] h-screen w-screen"
        style={{ background }}
      ></motionDiv.div>
      <motionDiv.div
        className="absolute left-0 top-0 h-[1200vh] w-screen"
        ref={mainWrapper}
      ></motionDiv.div>
      <motionDiv.h1
        style={{
      ...
      >
        <span className="bg-yellow-300">I'm a Front-end Developer</span>
      </motionDiv.h1>
      <Canvas
        style={{
          position: "fixed",
