...
const GraphicsPortfolio: React.FC = () => {
  ...
  const scale = useTransform(scrollYProgress, [0.1, 0.7, 1], [0, 2, 20]);
  const rotateX = useTransform(scrollYProgress, [0.1, 0.7, 1], [1, 5, 5.3]);
  const rotateY = useTransform(scrollYProgress, [0.4, 0.9], [0, 3]);
  const rotateZ = useTransform(scrollYProgress, [0.4, 0.9, 1], [0, 3, 6]);
  const donutPosX = useTransform(scrollYProgress, [0.1, 0.6], [3, 0]);
  const donutPosY = useTransform(scrollYProgress, [0.1, 0.8], [0, 0]);
  const donutPosZ = useTransform(scrollYProgress, [0.1, 0.8], [-1, 3]);

  const scale2 = useTransform(scrollYProgress, [0.1, 0.3, 1], [0, 2, 20]);
  const rotateX2 = useTransform(scrollYProgress, [0.1, 0.3, 1], [1, 5, 5.3]);
  const rotateY2 = useTransform(scrollYProgress, [0.4, 0.4], [0, 3]);
  const rotateZ2 = useTransform(scrollYProgress, [0.4, 0.4, 1], [0, 3, 6]);
  const donutPosX2 = useTransform(scrollYProgress, [0.1, 0.2], [-3, 0]);
  const donutPosY2 = useTransform(scrollYProgress, [0.1, 0.3], [0, 0]);
  const donutPosZ2 = useTransform(scrollYProgress, [0.1, 0.3], [1, 3]);

  const contentBgColor = useTransform(
    scrollYProgress,
    [0, 0.9, 1],
    ["0", "1", "0"]
  );
    ...
    <>
      ...
      >
        ...
        <Suspense fallback={<>Loading</>}>
          ...
          <ModelComponent
            scale={scale2}
            donutPosX={donutPosX2}
            donutPosY={donutPosY2}
            donutPosZ={donutPosZ2}
            rotateX={rotateX2}
            rotateY={rotateY2}
            rotateZ={rotateZ2}
            gltfPath={gltfPath2}
