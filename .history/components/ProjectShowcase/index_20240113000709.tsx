    ...
    <>
      ...
      >
        <motionDiv.h2
          ...
          className="bg-black bg-opacity-40 !z-[550] relative font-sans font-bold italic w-max top-12 lg:top-0 text-2xl lg:text-6xl text-white select-none text-center p-3 rounded-xl"
        >
          Projects Showcase
        </motionDiv.h2>
      </motionDiv.div>
        ...
        >
          {projects.map((item: any, i: any) => (
            <motionDiv.div
              ...
              className="min-h-[100vh] my-12 lg:my-0 flex items-center duration-75 py-3 px-6 lg:px-24"
            >
              <div className="w-full grid grid-cols-1 lg:grid-cols-2 h-max mt-72 lg:mt-0">
                <motionDiv.div
                  style={{
                    translateX: project1TranslateXLeft,
                  }}
                  className="mx-3 lg:mx-6 relative"
                >
                  <Link
                    href={item.link}
                    title="Open in new tab"
                    target="_blank"
                    className="absolute bottom-6 right-6 text-blue-400 hover:text-gray-400 hover:bg-slate-800 hover:bg-opacity-70 p-2 rounded-md duration-200 border-2 border-transparent hover:border-zinc-800 not-italic flex flex-row items-center mt-3 text-xl group"
                  >
                    <FaLink className="mr-2 text-gray-500 group-hover:text-blue-400 duration-200" />{" "}
                    Visit website
                  </Link>
                  <Image
                ...
                <motionDiv.div
                  className="italic mt-4 lg:mt-0 mx-3 lg:mx-6 text-white bg-slate-800 bg-opacity-70 overflow-hidden rounded-xl border-4 border-zinc-800 h-max"
                  style={{
                    translateX: project1TranslateXRight,
                    boxShadow: "0px 0px 6px black",
                  }}
                >
                  <h2 className="bg-yellow-300 w-full p-2 text-center mx-auto lg:mx-0 text-black font-sans font-bold text-5xl drop-shadow-md shadow-black px-1">
                    {item.name}
                  </h2>
                  <div className="p-3">
                    ...
                    <div className="flex flex-row items-start flex-wrap -ml-3">
                      {item.technologies.map((technology: any, i: any) => (
                        <p key={i} className="font-sans text-lg ml-3">
                          {technology}
                        </p>
                      ))}
                    </div>
                    <h2 className="font-bold text-2xl mt-3">Structure</h2>
                    <p>{item.structure}</p>
