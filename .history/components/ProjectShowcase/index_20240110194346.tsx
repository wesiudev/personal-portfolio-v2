...
export default function ProjectShowcase({ menuOpacity }: { menuOpacity: any }) {
  ...
  const project1Y = useTransform(scrollYProgress, [0.2, 0.3], ["100vh", "0vh"]);
  const project1Scale = useTransform(scrollYProgress, [0.2, 0.3], ["0.5", "1"]);
    ...
    <>
      ...
      >
        <div className="sticky top-0 z-[504] w-screen h-screen">
          <div className="top-0 lg:top-36 h-screen lg:h-max flex items-center overflow-x-hidden">
            <motionDiv.div
              style={{
                translateY: project1Y,
                scale: project1Scale,
              }}
              className="sticky flex items-end  h-[50vh] w-max duration-75 py-3 px-3"
            >
              <div className={`w-[88vw] lg:w-[70vw] xl:w-[40vw] rounded-xl`}>
                <Image
