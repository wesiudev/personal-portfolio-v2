      ...
      <div className="relative h-max">
          ...
          <button
            onClick={() =>
              setCurrentImage({
                project: item.name,
                index:
                  currentImage.index >= 0 && currentImage <= item.images.length
        ...
        <div className="h-max flex flex-row space-x-2 lg:space-x-4 bottom-3 left-1/2 -translate-x-1/2 absolute bg-black bg-opacity-60 p-1.5">
          {project.colors.map((color: any, i: number) => (
            <li key={i} className="flex flex-row projects-center">
              <div
        ...
        {project.images.map((image: any, i: any) => (
          <>
            {currentImage.index === i &&
              currentImage.project === project.name && (
                <Image
                  key={i}
        ...
        {currentImage.index === 0 && currentImage.project === "" && (
          <Image
            style={{ boxShadow: "0px 0px 6px black" }}
