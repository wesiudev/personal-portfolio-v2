  ...
  return (
    <>
      <motionDiv.h2
        ...
        className="hidden lg:block bg-black bg-opacity-40 z-[550] fixed left-1/2 font-sans w-max top-0 text-2xl lg:text-3xl xl:text-4xl text-white select-none text-center p-3 rounded-xl"
      >
        Projects Showcase
      </motionDiv.h2>
      <div
        ref={mainWrapper}
        className="flex-col flex w-screen relative mt-[525vh]"
      >
        <motionDiv.h2
          style={{
            opacity: h1TextOpacity,
            translateY: h1TextTranslateX,
          }}
          className="lg:hidden bg-black bg-opacity-40 z-[550] fixed right-6 font-sans w-max -top-3 text-2xl lg:text-3xl xl:text-4xl text-white select-none text-center p-3 rounded-xl"
        >
          Projects Showcase
        </motionDiv.h2>
        <motionDiv.div className="z-[504] top-0 w-screen flex flex-col mt-36 lg:mt-36">
          {projects.map((item: any, i: any) => (
            <motionDiv.div
              key={i}
              className="h-max py-36 my-12 lg:my-0 flex items-center duration-75 px-6 lg:px-12"
            >
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 mt-3 lg:mt-1.5 xl:mt-3">
                      {item.colors.map((color: any, i: number) => (
                        <li key={i} className="flex flex-row items-center">
                          ...
                          ></div>
                          <span className="not-italic">{color}</span>
                        </li>
                      ))}
                    </div>
                    <h2 className="font-bold text-lg lg:text-xl xl:text-2xl mt-3 lg:mt-1.5 xl:mt-3">
